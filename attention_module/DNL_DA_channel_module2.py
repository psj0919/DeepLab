...
class DNL_DA(nn.Module):
    def __init__(self, inplane):
        ...
        self.layer5_conv = nn.Conv2d(in_channels=inplane, out_channels=inplane, kernel_size=1)
    ...
    def forward(self, x):
        ...
        # attention2
        proj_query2 = self.layer3_conv(x).view(batch_size, channel, -1)
        proj_key2 = self.layer4_conv(x).view(batch_size, channel, -1).permute(0, 2, 1)
        ...
        proj_value = self.layer5_conv(x).view(batch_size, channel, -1)
